...
export function generateThreeSumSteps(
  ...
  // For sorting metrics, let's assume N log N comparisons and operations roughly
...
export function runThreeSumExperiment(inputSize: number): ComplexityMetrics {
  const { nums, target } = generateRandomThreeSumInput(inputSize);
